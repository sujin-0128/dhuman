...
<form id="frmGoods" name="frmGoods" action="./discount_bundle_group_ps.php" method="post" enctype="multipart/form-data">
    ...
    <table cellpadding="0" cellpadding="0" width="100%" id="tbl_add_goods_set" class="table table-rows">
        ...
        <tr id="goodsRegisteredTrArea">
            ...
            <!-- <th class="width15p">결합실패 시</th> -->
        </tr>
        </thead>
        <tbody>
        <?php
        if (count(gd_isset($discountBundleGroupGoodsList))) {
            foreach ($discountBundleGroupGoodsList as $key => $val) {
                ...
                <tr id="tbl_add_goods_<?=$val['goodsNo'];?>" class="add_goods_free">
                    ...
                    <!-- <td class="center">
                        <select name="bundleType[]">
                            ...
                            <option value="main" <?php if($val['bundleType'] == 'main'){echo "selected";} ?> >판매함</option>
                            <option value="discount" <?php if($val['bundleType'] == 'discount'){echo "selected";} ?> >판매안함</option>
                        </select>
                    </td> -->
                </tr>
                <?php
    ...
    <div class="table-action">
        ...
        <div class="pull-right">
           <button class="checkRegister btn btn-white" type="button"  onclick="goods_search_popup()">상품 불러오기</button>
        </div>
    </div>
</form>
