$(function(){
	...
	//240131 - [ board - iframe 16:9 비율에 필요한 class 부여 ]
	$('.board_view_content .seem_cont iframe').parent().addClass('iframe21to9')
	...
	$(window).on('scroll',function(){
		...
		let fixedTop = $(window).scrollTop();
		let fixedHeight = $('#footer_wrap').offset().top;
		let elementHeight = $('#footer_wrap').height();   
		

		if(fixedHeight < (viewportHeight+fixedTop)){
			$('#scroll_right').addClass('foot_fixed').removeClass('ban_fixed')
		}else{
			$('#scroll_right').removeClass('foot_fixed')			
			if(fixedTop > 650){
				$('#scroll_right').addClass('ban_fixed')
			}else{
				$('#scroll_right').removeClass('ban_fixed')
			}
		}
	})
	*/
...
$(document).ready(function() {
	...
	$(document).click(function(e) {
		if($('.btn_all_menu_open').hasClass('on')) {
			if(!$(e.target).closest('#header').length) {
				e.preventDefault();
				gd_btn_all_menu_close();
				$('.btn_all_menu_open').removeClass('on');
			}
		}
...
/* 스크롤배너(오른쪽)
	230918 - ban_fixed 클래스가 윈도우 크기 줄이면 사라지는 현상이 있어 주석처리 해 놓았음.
*/
        ...
        $(window).on({
            ...
            'scroll':function(){
                ...
                if(scqTop <= quickTop){
					...
					if(el.eq(1).outerHeight() + scqTop >= $('#footer_wrap').offset().top) {
						el.css({
							'top' : ($('#footer_wrap').offset().top - (el.eq(1).outerHeight() + scqTop))+'px'
						})
					}
                }
            }
...
$(window).scroll(function() {
	...
	var gnbX = $('.header_search').height() + $('.header_top').height();

	if(scrollX >= gnbX) {
		$('.header_gnb').addClass('fixed');
		$('#header').css('padding-bottom', $('.header_gnb').height()+'px');
		$('#header').addClass('on');
	} else {
