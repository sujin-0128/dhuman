 ...
 */
class OrderPsController extends \Bundle\Controller\Mobile\Order\OrderPsController
{
    /**
     * {@inheritdoc}
     *
        ...
        switch ($postValue['mode']) {
            ...
            case 'orderNew':
                try {
					...
					//jdev.20240502.s	
					if(\Request::post()->get('tmpDeliverTab') =='direct'){
						if($postValue['directDefault'] == 'y'){
							$postValue['reflectApplyDelivery'] = 'y';
						}
						$postValue['shippingDefault'] = 'n';
					}
					//jdev.20240502.e	
...
//                    }
                    ...
                    \Logger::channel('order')->info(__METHOD__ . ' getCartGoodsData() result : ', [$cartInfo]);
                    ...
                    if ($click_key != '' && $click_key != null && $goodsNo != '' && $goodsNo != null) {
                        ...
                        $this->db->query("INSERT INTO `es_adInfo` (`orderNo`, `goodsNo`, `adChannel`, `adKey`, `memo`, `result`, `regDt`) VALUES ('".$order->orderNo."', '".$goodsNo."', 'AdiSON', '".$click_key."', 'OrderPsM', '', now());");
                    }
                                        
                    // 주문로그 저장
                    \Logger::channel('order')->info(__METHOD__ . ' OREDR NO : ' . $order->orderNo, $postValue);
            ...
            default:
                try {
                    ...
                    if (\Request::getRemoteAddress() === '182.216.219.157' || \Request::getRemoteAddress() === '220.118.145.49'){
