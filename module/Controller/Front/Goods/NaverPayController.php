...
class NaverPayController extends \Bundle\Controller\Front\Controller
{
    ...
    public function index()
    {
		...
		//desingpix 링크프라이스 튜닝
		$orderInfo = $order->getOrderData($orderNo);
		if($orderInfo['lpinfo']){	
			$lp = \App::load('\\Component\\Designpix\\Lp');

			$lpinfo['lpinfo'] = $orderInfo['lpinfo'];
			$lpinfo['user_agent'] = $orderInfo['user_agent'];
			$lpinfo['remote_addr'] = $orderInfo['orderIp'];
			$lpinfo['device_type'] = $orderInfo['orderTypeFl'];	
							
			$lp->orderPaid($orderNo, $lpinfo, 'naverpay');
		}
            ...
            foreach ($orderGoodsList as $orderGoodsData) {
                ...
                if (count($addOrderGoodsData[$orderGoodsData['sno']][$goodsNo]) > 0) {
                    ...
                    foreach ($arrAddGoods as $val) {
                        ...
                        $arr_data[] = '<supplement>';
                        //                        debug($orderGoodsData,true);
                        if ($addGoodsTuningFl) {
                            $arr_data[] = '<id>addGoods'.'_'.$val['parentGoodsNo'].'_'.$val['goodsNo'].'</id>';
                        } else {
                ...
                if ($isNotOption) { //단품
                    ...
                    // S : 튜닝여부 확인 *** 수정 금지 ***
                    if($tuningFl) {
                        $arr_data[] = '<manageCode>option' . $orderGoodsData['optionSno'] . '</manageCode>';
                    } else {
 ...
 requiresDOM: "non-blocking", cb: function () {
  addEvent("npay_btn_pay", "click", function () {
   if (typeof(Storage) !== "undefined") {                        
    criteoNpayEvent(74506, [{ id: "<?=$goodsNoArr[0]?>", price: <?=$goodsPrice?>, quantity: 1}], "", "");
   } 
  });                
 }
});
...
function criteoNpayEvent(parterId, itemArray, email="", zipcode="") {
 ...
 criteo_q.push(
  ...
  { event: "trackTransaction", id: "npay" + Math.floor(Math.random()*99999999999), item: [
  {id: "<?=$goodsNoArr[0]?>", price: <?=$goodsPrice?>, quantity: 1 },
]}
 );
